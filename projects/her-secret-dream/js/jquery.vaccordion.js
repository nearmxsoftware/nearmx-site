					...
					function( dfd ) {
						...
						$el.stop()
						   .animate( animParam, settings.animSpeed, settings.animEasing, function() {
								if( !expanded )
									$el.find('.hsd-contenido').fadeIn( settings.contentAnimSpeed );
						   })
						   .find('.hsd-titulo')
						   .stop()
						   .animate({
						...
						if( i === $nextSlice.index() ) {
							...
							$slice.stop()
						          .animate({
										...
										$slice.find('.hsd-contenido').fadeIn( settings.contentAnimSpeed );
								  })
								  .find('.hsd-titulo')
								  .stop()
								  .animate({
				...
				if( this.length ) {
					...
					return this.each(function() {
						...
						var $el 			= $(this),
							...
							$slices			= $el.find('div.hsd-slice'),
							// the navigation buttons
							$navNext		= $el.find('span.hsd-nav-siguiente'),
							$navPrev		= $el.find('span.hsd-nav-anterior');
